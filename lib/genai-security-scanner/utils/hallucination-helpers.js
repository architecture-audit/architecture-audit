...
class HallucinationHelpers {
    ...
    /**
     * Calculate information gain between response and answer
     */
    calculateInformationGain(responseTokens, answerTokens) {
        if (!responseTokens || !answerTokens) return 0;

        const responseSet = new Set(responseTokens);
        const answerSet = new Set(answerTokens);

        // Calculate overlap
        const intersection = new Set([...responseSet].filter(x => answerSet.has(x)));
        const union = new Set([...responseSet, ...answerSet]);

        // Calculate Jaccard similarity as proxy for information gain
        if (union.size === 0) return 0;

        const jaccard = intersection.size / union.size;

        // Calculate information gain based on entropy reduction
        const responseEntropy = Math.log2(responseSet.size + 1);
        const answerEntropy = Math.log2(answerSet.size + 1);
        const jointEntropy = Math.log2(union.size + 1);

        // Information gain = H(Response) + H(Answer) - H(Response, Answer)
        const gain = responseEntropy + answerEntropy - jointEntropy;

        return Math.max(0, gain) * jaccard;
    }
