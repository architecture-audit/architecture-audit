// LLM Cache Loader for Databricks Recommendations
// Integrates with existing recommendation system

class DatabricksLLMCache {
    constructor() {
        this.cache = null;
    ...
    async init() {
        try {
            ...
            console.log('LLM cache loaded successfully');
            return true;
        } catch (error) {
            console.warn('LLM cache not available, using template-based recommendations', error);
            return false;
        }
    }
    ...
    async fetchCache() {
        try {
            ...
            const paths = [
                '/public/data/databricks-llm-cache.json',  // Local development
                '/data/databricks-llm-cache.json',         // Production CDN
                './data/databricks-llm-cache.json'         // Relative path
            ];
            ...
            for (const path of paths) {
                try {
                    ...
                    if (response.ok) {
                        console.log('Loaded LLM cache from:', path);
                        break;
                    }
                } catch (e) {
                    lastError = e;
            ...
            try {
                localStorage.setItem('databricks-llm-cache', JSON.stringify(data));
                localStorage.setItem('databricks-llm-cache-version', data.version);
            } catch (e) {
            ...
            const stored = localStorage.getItem('databricks-llm-cache');
            if (stored) {
                try {
    ...
    formatRecommendation(cached, inputs) {
        ...
        return {
            ...
            explanation: rec.explanation || ['LLM-generated recommendation'],
    ...
    adjustRecommendation(cached, inputs) {
        ...
        // Mark as adjusted
        recommendation.source = 'llm-adjusted';
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = DatabricksLLMCache;
}
if (typeof window !== 'undefined') {
    window.DatabricksLLMCache = DatabricksLLMCache;
}