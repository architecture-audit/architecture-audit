...
class LLMConnector {
    ...
    async initializeOpenAI() {
        try {
            ...
            // Check if API key exists before trying to initialize
            if (!apiKey) {
                console.warn('No OpenAI API key found. Using mock mode.');
                this.initializeMock();
                return;
            }
    ...
    async initializeMock() {
        console.log('🔧 Initializing mock mode (no API key)');
        ...
        this.queryFunction = async (prompt) => {
            ...
            let response = `Mock response to: ${prompt.substring(0, 50)}...`;

            // Add some variety based on prompt type
            if (prompt.toLowerCase().includes('model')) {
                response = 'I am a mock LLM model for testing purposes.';
            } else if (prompt.toLowerCase().includes('capital')) {
                response = 'Paris';
            } else if (prompt.toLowerCase().includes('2+2') || prompt.toLowerCase().includes('2 + 2')) {
                response = '4';
            } else if (prompt.toLowerCase().includes('ignore') || prompt.toLowerCase().includes('injection')) {
