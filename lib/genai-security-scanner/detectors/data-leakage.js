...
class DataLeakageDetector {
    ...
    detectPII(text) {
        const detected = [];
        // Perform enhanced PII analysis
        const enhancedAnalysis = {
            hasCreditCard: /\b(?:\d[ -]*?){13,16}\b/.test(text),
            hasSSN: /\b\d{3}-\d{2}-\d{4}\b/.test(text),
            hasPhone: /(\+\d{1,3}[-.\s]?)?\(?\d{1,4}\)?[-.\s]?\d{1,4}[-.\s]?\d{1,9}/.test(text),
            hasEmail: /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/.test(text),
            hasIPAddress: /\b(?:\d{1,3}\.){3}\d{1,3}\b/.test(text),
            hasMedicalID: /\b[A-Z]\d{7,12}\b/.test(text),
            hasPassport: /\b[A-Z]{1,2}\d{6,9}\b/.test(text),
            hasDriverLicense: /\b[A-Z]{1,2}\d{5,12}\b/.test(text),
            hasBankAccount: /\b\d{8,17}\b/.test(text),
            hasAddress: /\d+\s+[A-Za-z]+\s+(Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Court|Ct|Boulevard|Blvd)/i.test(text)
        };
